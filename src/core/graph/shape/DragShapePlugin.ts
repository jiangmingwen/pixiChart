...
export class DragShapePlugin extends GraphPlugin {
    ...
    private calculatePosition(e: FederatedPointerEvent) {
        ...
        if (!e.ctrlKey && !e.altKey) {
            ...
            console.log(parentId,parent,'parent')
            if (parent) {
            ...
            if (parent) {
                x = clamp(x, GlobalStype.SafePadding, parent.width - this.baseShape!.width - GlobalStype.SafePadding)
                y = clamp(y, GlobalStype.SafePadding, parent.height - this.baseShape!.height - GlobalStype.SafePadding)
            }
        }
